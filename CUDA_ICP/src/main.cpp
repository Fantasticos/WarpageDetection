...
void parseModel() {
	int counter = 0;
	std::ifstream waymoFile("../object_dsp.pcd");
  std::ifstream waymoFile2("../scene_dsp.pcd");
  ...
  N1=counter;
  counter=0;
  while (std::getline(waymoFile2, line)) {
		//std::cout << line << std::endl;
		std::stringstream ss(line);
		ss >> x;
		ss >> y;
...
#elif GPU_NAIVE
	ScanMatch::initSimulationGPU(N1,N2,coords,coords2);
#elif GPU_OCTREE
	ScanMatch::initSimulationGPUOCTREE(N1,N2,coords,coords2);
#endif
/*
  updateCamera();
...
/*
	...
	std::cout << "ICP Time: "<<duration.count() <<" ms"<< std::endl;
    icpTfMat = ScanMatch::getTfMat();
    std::cout<<"ICP transformation matrix"<<std::endl;
    std::cout<<icpTfMat[0].x<<" "<<icpTfMat[1].x<<" "<<icpTfMat[2].x<<" "<<icpTfMat[3].x<<"\n";
    std::cout<<icpTfMat[0].y<<" "<<icpTfMat[1].y<<" "<<icpTfMat[2].y<<" "<<icpTfMat[3].y<<"\n";
    std::cout<<icpTfMat[0].z<<" "<<icpTfMat[1].z<<" "<<icpTfMat[2].z<<" "<<icpTfMat[3].z<<"\n";
    std::cout<<icpTfMat[0].w<<" "<<icpTfMat[1].w<<" "<<icpTfMat[2].w<<" "<<icpTfMat[3].w<<"\n";
/*
      std::ostringstream ss;
      ss << "[";
      ss.precision(1);
